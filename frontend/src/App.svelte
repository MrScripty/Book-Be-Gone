...
<div class="app-shell">
  ...
  {:else}
    <div class="workflow-bar">
      ...
      <div class="ocr-controls"><details class="menu settings-menu"><summary aria-label="OCR settings">{session.modelId.replace(/^gpt-/, '')||'Model'} <span class="muted">/ {levels[session.effort]}</span><span class="chevron">⌄</span></summary><div class="menu-content settings"><label>Model<select aria-label="OCR model" bind:value={session.model} disabled={session.running} onchange={()=>session.setModel()}>{#each session.models as model}<option value={model.id}>{model.name}</option>{/each}<option value="custom">Custom model…</option></select></label>{#if session.model==='custom'}<input aria-label="Custom OCR model identifier" placeholder="Model identifier" bind:value={session.custom} disabled={session.running} onchange={()=>session.setModel()}/>{/if}<label>Thinking<select aria-label="Thinking level" bind:value={session.effort} disabled={session.running} onchange={()=>session.setEffort()}>{#each session.efforts as effort}<option value={effort}>{levels[effort]||effort}</option>{/each}</select></label></div></details>
